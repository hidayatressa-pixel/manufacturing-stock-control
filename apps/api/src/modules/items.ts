...
itemRouter.patch("/:id",requireCapability("item:write"),async(req,res,next)=>{try{const data=itemInput.partial().parse(req.body);res.json(await prisma.item.update({where:{id:String(req.params.id)},data}));}catch(e){next(e)}});
itemRouter.delete("/:id",requireCapability("item:write"),async(req,res,next)=>{try{const id=String(req.params.id),used=await prisma.stockLedger.count({where:{itemId:id}});if(used)return res.status(409).json({message:"Item has transaction history. Deactivate it instead."});await prisma.item.delete({where:{id}});res.status(204).end();}catch(e){next(e)}});
